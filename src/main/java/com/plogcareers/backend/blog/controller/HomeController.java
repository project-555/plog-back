...
public class HomeController {
    ...
    public ResponseEntity<Void> createSubscribe(@ApiIgnore @RequestHeader(name = Auth.TOKEN) String token,
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "정상 조회", response = ListSubscribesResponse.class),
            @ApiResponse(code = 500, message = "서버 에러", response = ErrorResponse.class)
    }
    )
    @GetMapping("/{userID}/subscribe")
    public ResponseEntity<ListSubscribesResponse> listSubscribes(@ApiParam(name = "userID", value = "유저 ID") @PathVariable Long userID) {
        return ResponseEntity.status(HttpStatus.OK).body(homeService.listSubscribes(userID));
    }

    @ApiOperation(value = "홈 포스팅 리스트(구독)")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "정상 조회", response = ListHomePostingsResponse.class),
            @ApiResponse(code = 400, message = "잘못된 요청", response = ErrorResponse.class),
            @ApiResponse(code = 500, message = "서버 에러", response = ErrorResponse.class)}
    )
    @GetMapping("/following-postings")
    public ResponseEntity<ListHomePostingsResponse> listFollowingPostings(@ApiIgnore @RequestHeader(name = Auth.TOKEN, required = false) String token,
                                                                          ListFollowingPostingsRequest request) {
        Long loginedUserID = authService.getLoginedUserID(token);
        return ResponseEntity.status(HttpStatus.OK).body(homeService.listFollowingPostings(loginedUserID, request.getSearch(), request.getLastCursorID(), request.getPageSize()));
    }
    ...
    public ResponseEntity<ListHomePostingsResponse> listRecentPostings(ListRecentPostingsRequest request) {
        return ResponseEntity.status(HttpStatus.OK).body(homeService.listRecentPostings(request.getSearch(), request.getLastCursorID(), request.getPageSize()));
    }
}
